...
private:
    ...
    mutable Lock _lock;
    std::unordered_map<uint32_t, std::string> _fontMap;
    bool _refreshWidgetMenu{false};
    bool _bVisible{true};

public:
    static std::string SanitizeTextForFont(const std::string& _text, const std::string& _font) {
        if (_font.find("Falmer") != std::string::npos ||
            _font.find("Dragon") != std::string::npos ||
            _font.find("Dwemer") != std::string::npos ||
            _font.find("Daedric") != std::string::npos ||
            _font.find("MageScript") != std::string::npos) {
